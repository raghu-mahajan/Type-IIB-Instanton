...
\end{align}
The free-field operator products fix the normalization of $\TF$ only up to
an overall sign.  We fix that sign relative to the field order in
Eq.~\eqref{eq:TF-free-definition} and the picture-changing convention in
Eq.~\eqref{eq:pco-expanded}.  Here $e^{q\phi}$ denotes the exponential of the
bosonized superghost together with its cocycle factor.  The fields $\TF$ and
$e^{-\phi}$ are both Grassmann odd; the cocycle convention is stated fully
in the paragraph preceding Eq.~\eqref{eq:phi-provisional-definition}.  Equations
\eqref{eq:psi-psi-ope} and \eqref{eq:TF-free-definition} give
\begin{align*}
\TF(z)\psi^M(w)
&\sim
-\frac{\partial X^M(w)}{2(z-w)},
\\
2e^\phi(z)\TF(z)e^{-\phi}(w)\psi^M(w)
&=
-2e^\phi(z)e^{-\phi}(w)\TF(z)\psi^M(w)
\\
&\sim
-2(z-w)\left[-\frac{\partial X^M(w)}{2(z-w)}\right]
\\
&=
\partial X^M(w).
\end{align*}
The minus sign in the second line is the graded sign obtained by moving the
odd field $e^{-\phi}(w)$ to the left of the odd field $\TF(z)$.  Thus the
picture-changing operator maps the $-1$-picture factor
$e^{-\phi}\psi^M$ to $\partial X^M$ with positive coefficient.  The
superconformal operator products of $\Tmat$ and $\TF$ are derived in
Sec.~\ref{sec:super-virasoro}.
...
The scalar $\phi$ is Grassmann even.  The symbol $e^{q\phi}$ denotes its
exponential together with the cocycle factor, namely the $z$-independent
factor that supplies the Grassmann parity required by the bosonization.  For
integer $q$, we define
\[
...
Grassmann parity is assigned only to the complete Ramond operator containing
$e^{q\phi}$ and a spin field, rather than to $e^{q\phi}$ by itself.  For
$q_1,q_2\in\tfrac12\mathbb Z$, the scalar and exponential operator products
used below are
\begin{equation}
\begin{aligned}
...
\end{align}
The cocycle convention preceding Eq.~\eqref{eq:phi-provisional-definition}
makes both $e^{-\phi}$ and $e^\phi$ Grassmann odd.  Consequently, each
...
Let $T(z)$ be the stress tensor and $\TF(z)$ be its supercurrent in the normalization defined below,  and let $c$ be its central charge.
We define the normalization of $T$ and $\TF$ by the singular operator products
\begin{align}
T(z)T(w)
&\sim
\frac{c/2}{(z-w)^4}
...
\label{eq:TF-TF-ope}
\end{align}
Equations~\eqref{eq:TT-ope}--\eqref{eq:TF-TF-ope} match the

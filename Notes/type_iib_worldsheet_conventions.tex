...
Euclidean worldsheet coordinates are denoted by $z$ and $\bar z$.
Target-space vector indices $M,N$ take values in $\{0,1,\ldots,9\}$.
...
$c$ be the central charge.
We define the normalization of these operators by the following OPEs
\begin{align}
T(z)T(w)
&\sim
\frac{c/2}{(z-w)^4}
...
\item
A Grassmann parity of an operator $A$ is denoted by $|A|\in\{0,1\}$.
\end{enumerate}

\subsection{Conventions for the \texorpdfstring{$bc$}{bc} ghost system}
\label{sec:bc-system}

The reparametrization ghosts $b(z)$ and $c(z)$ are Grassmann odd.
Their holomorphic conformal weights and ghost numbers are
\begin{center}
\begin{tabular}{ccc}
\toprule
...
\end{align}
Differentiation of Eq.~\eqref{eq:xi-eta-ope} gives
\begin{align*}
\partial\xi(z)\eta(w)
&\sim-\frac{1}{(z-w)^2},
\\
...
Let $X^M(z,\bar z)$ be the ten embedding coordinates.
...
\label{eq:dX-dX-ope}
\end{align}
The holomorphic stress tensor of the ten bosons
is defined by
\begin{equation}

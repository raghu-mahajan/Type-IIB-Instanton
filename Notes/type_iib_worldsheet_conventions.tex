...
\begin{abstract}
These notes fix the holomorphic worldsheet conventions used below for flat
ten-dimensional type IIB string theory.  The normalization of the free
fields, the ghost systems, the BRST current, and the picture-changing
operator follows Ref.~\cite{Barman:2026qzi}.  In particular, the
picture-changing operator is twice the normalization often used in the
literature.  The ten-dimensional gamma-matrix orientation and the
normalization of the Ramond spin-field operator products follow
Ref.~\cite{Alexandrov:2021dyl}.  Every displayed holomorphic formula has an
anti-holomorphic counterpart obtained by placing a tilde on each field and
replacing $z,w$ by $\bar z,\bar w$.
\end{abstract}
...
Normal ordering is defined by point splitting:
\begin{equation}
:\!AB\!:(w)
:=
\lim_{z\to w}
...
holomorphic conformal weights, ghost numbers, and singular operator product
are
\begin{equation}
\begin{array}{c|cc}
\text{field}&h&\text{ghost number}\\
\hline
b&2&-1\\
c&-1&+1
\end{array}
...
on the elementary bosonized fields by
\begin{equation}
\begin{array}{c|ccccc}
\text{field}&\beta&\gamma&\xi&\eta&e^{q\phi}\\
\hline
\text{ghost number}&-1&+1&-1&+1&0\\
\text{picture number}&0&0&+1&-1&q
\end{array}.
\label{eq:superghost-charges}
\end{equation}
...
The zeroth-order normal product $:\!\psi^M\psi_M\!:$ vanishes, while the
...
\end{equation}
All products on the right-hand side of Eq.~\eqref{eq:brst-current} are
normal ordered.  In bosonized variables, the composite field $\gamma^2$ is
\begin{equation}
\gamma^2:=:\!\gamma\gamma\!:
